...
Ancestors
  string mlstring option sum pair list alist rich_list arithmetic pred_set
...
Definition compile_input_rel_def:
  compile_input_rel (s :mlstring) (ws :word8 list) ⇔
    ∃junk.
      strlen junk = max_FFI_return_size - strlen s ∧
      ws = [n2w (strlen s); n2w (strlen s DIV 256)] ++
              MAP (λc. n2w (ORD c)) (explode $ s ^ junk)
End
...
Proof
  rw[store_lookup_def, semanticPrimitivesTheory.store_assign_def,
...
Proof
  rw[env_ok_def, var_prefix_def, strcat_def, mlstringTheory.concat_def]
QED
...
Proof
  ...
  >- (
    ...
    first_x_assum drule >>
    simp[var_prefix_def, strcat_def, mlstringTheory.concat_def]
    )
  >- (
...
Proof
  ...
  gvs[env_ok_def, var_prefix_def, strcat_def, mlstringTheory.concat_def] >>
  rw[] >>
  irule_at Any EQ_REFL >> simp[]
QED
...
val _ = (max_print_depth := 10000);

(***** primitive operations *****)

Theorem mlstring_DROP_CONS_EL:
  ∀n l. n < strlen l ⇒ DROP n (explode l) = EL n (explode l)::DROP (SUC n) (explode l)
Proof
  rw [explode_def] >>
  Cases_on ‘l’ >> gvs [] >>
  gvs [explode_aux_thm, DROP_CONS_EL]
QED

val strle_step_tac =
  qrefine ‘SUC _’ >>
  simp [cstep, do_app_def, do_opapp_def, strle_v_def, find_recfun_def,
        do_test_def, check_type_def, do_conversion_def, do_eq_def,
        lit_same_type_def, do_arith_def];
...
Theorem strle_lemma:
  ...
  ⇒ ∃k env'. cstep_n k (Estep (env,st,Exp (var «strle»),
      ...
      Estep (env',st,Val (Boolv (DROP n (explode s1) ≤ DROP n (explode s2))),c)
Proof
  recInduct strle_ind >> rw[] >>
  ntac 17 strle_step_tac >>
  ...
  >- (
    ...
    ‘DROP n (explode s1) = ""’ by rw [DROP_EQ_NIL] >> simp []) >>
  ntac 7 strle_step_tac >>
  simp [do_if_def] >> IF_CASES_TAC >> gvs []
  >- (
    strle_step_tac >>
    simp [do_con_check_def, build_conv_def] >>
    qexists0 >> simp [Boolv_def] >> IF_CASES_TAC >> simp [] >>
    pop_assum mp_tac >> simp [] >>
    ‘DROP n (explode s2) = ""’ by rw [DROP_EQ_NIL] >> simp [] >>
    Cases_on ‘DROP n (explode s1)’ >> gvs []) >>
  gvs [NOT_LESS_EQUAL] >> imp_res_tac mlstring_DROP_CONS_EL >> gvs[] >>
  ntac 25 strle_step_tac >>
  simp [do_if_def] >> IF_CASES_TAC >> gvs [ORD_11]
  >- (
    strle_step_tac >>
    simp [do_con_check_def, build_conv_def] >>
  ...
  ntac 7 strle_step_tac >>
  simp [do_if_def] >> reverse IF_CASES_TAC >> gvs []
  >- (
    strle_step_tac >>
    simp [do_con_check_def, build_conv_def] >>
    qexists0 >> simp [Boolv_def, char_lt_def] >> gvs [ORD_11]) >>
  ntac 19 strle_step_tac >>
  simp [char_lt_def] >> gvs [ORD_11, ADD1] >>
  gvs[integerTheory.INT_ADD_CALCULATE] >>
  first_x_assum irule >> gvs [] >>
  simp [build_rec_env_def, strle_v_def, oneline strsub_def] >>
  rpt $ TOP_CASE_TAC >> gvs []
QED

Theorem string_le_explode:
  string_le (explode s1) (explode s2) ⇔ s1 ≤ s2
Proof
  rw [explode_def] >>
  Cases_on ‘s1’ >> Cases_on ‘s2’ >> gvs [] >>
  gvs [explode_aux_thm, DROP_CONS_EL] >>
  qid_spec_tac `s'` >> qid_spec_tac `s` >> gvs [] >>
  Induct >> rw [] >> gvs [strlit_le_strlit]
QED
...
Theorem strle:
  ...
  ⇒ ∃k env'. cstep_n k (Estep (env,st,Exp (var «strle»),
        ...
        (Capp Opapp [Litv (StrLit s1)] [], env)::
        (Capp Opapp [Litv (StrLit s2)] [], env)::
        (Capp Opapp [Litv (IntLit &strlen s1)] [], env)::
        (Capp Opapp [Litv (IntLit &strlen s2)] [], env)::c)) =
      Estep (env',st,Val (Boolv (s1 ≤ s2)),c)
Proof
  rw[] >> drule_all $ SRULE [] strle_lemma >>
  disch_then $ qspec_then `0` mp_tac >> simp[string_le_explode]
QED
...
Theorem mlstring_gt_le:
  ∀s1 s2. mlstring_gt s1 s2 ⇔ ¬mlstring_le s1 s2
Proof
  rw[mlstring_gt_def] >>
  Cases_on ‘s1’ >> Cases_on ‘s2’ >> gvs [] >>
  qid_spec_tac ‘s’ >> qid_spec_tac ‘s'’ >> Induct >> gvs [mlstring_le_def]
QED
...
Proof
  ...
  >- ( (* ForceMutK *)
    ...
    qexists0 >> simp[step_rel_cases, SF SFY_ss] >>
    reverse $ rw[store_assign_def,semanticPrimitivesTheory.store_assign_def]
  ...
  >- ( (* AppK *)
    ...
    >- (
      ...
      gvs[bad_thunk_update_def] >>
      TOP_CASE_TAC >> gvs[] >>
      qpat_x_assum ‘store_assign _ _ _ = SOME _’ mp_tac >>
      simp[store_assign_def, store_same_type_def] >>
    ...
    >- ( (* opb *)
      gvs[eval_op_SOME] >>
      simp[cstep, do_app_def, do_test_def] >>
      qexists0 >> rw[step_rel_cases, Boolv_def] >> rpt $ goal_assum $ drule_at Any >>
      gvs [opb_rel_cases] >>
      gvs[env_rel_def, cnenv_rel_def, prim_types_ok_def]
      )
    >- ( (* opn *)
      ...
      simp[cstep, do_app_def, check_type_def, do_arith_def] >>
      qexists0 >> rw[step_rel_cases, SF SFY_ss]
      )
    )
  >- ( (* TwoArgs - second argument to evaluate *)
    ...
    irule_at Any env_ok_nsBind_alt >>
  ...
  >- ( (* TwoArgs - ready to evaluate *)
    ...
    >- ( (* Elem str *)
      gvs[eval_op_SOME] >>
      ntac 6 (qrefine `SUC n` >> simp[cstep_n_def, cstep]) >>
      simp[do_app_def, do_test_def, str_el_def] >>
      ...
      simp[do_app_def, do_test_def] >> reverse $ IF_CASES_TAC >> gvs[] >>
      ntac 2 (qrefine `SUC n` >> simp[cstep_n_def, cstep, do_if_def])
      >- (qexists0 >> simp[step_rel_cases, SF SFY_ss]) >>
      ...
      simp[do_app_def, check_type_def, do_conversion_def] >>
      qexists0 >> simp[step_rel_cases] >>
      rpt $ goal_assum $ drule_at Any >> simp[IMPLODE_EXPLODE_I, ORD_11] >>
      `ABS idx = idx` by ARITH_TAC >> simp[] >>
      simp [oneline strsub_def] >> TOP_CASE_TAC >> gvs [])
    >- ( (* Substring2 *)
      gvs[eval_op_SOME] >> rename1 `idx < _` >>
      ntac 4 (qrefine `SUC n` >> simp[cstep_n_def, cstep]) >>
      simp[do_app_def] >>
      ntac 8 (qrefine `SUC n` >> simp[cstep_n_def, cstep]) >>
      simp[do_app_def, do_test_def] >> IF_CASES_TAC >> gvs[] >>
      ntac 2 (qrefine `SUC n` >> simp[cstep_n_def, cstep, do_if_def])
      >- (
        ntac 7 (qrefine `SUC n` >> simp[cstep_n_def, cstep, do_if_def]) >>
        ...
        qexists0 >> simp[step_rel_cases] >>
        gvs [oneline strlen_def, AllCaseEqs()] >>
        TOP_CASE_TAC >> gvs [implode_def] >> simp [SF SFY_ss]) >>
      ...
      >- (
        ...
        simp[] >> qexists0 >>
        simp[step_rel_cases, implode_def, SF SFY_ss]) >>
      ntac 5 (qrefine `SUC n` >> simp[cstep_n_def, cstep]) >>
      simp[do_app_def, check_type_def, do_arith_def] >>
      ntac 5 (qrefine `SUC n` >> simp[cstep_n_def, cstep]) >>
      simp[do_app_def, copy_array_def, IMPLODE_EXPLODE_I] >>
      ...
      qexists0 >> simp[step_rel_cases] >> rpt $ goal_assum $ drule_at Any >>
      `ABS idx = idx` by ARITH_TAC >> simp[] >>
      Cases_on ‘s''’ >> gvs [implode_def] >>
      ARITH_TAC)
    >- ( (* StrLeq *)
      gvs[eval_op_SOME] >>
      rename1 ‘nsLookup _ _ = SOME (Litv (StrLit s1))’ >>
      rename1 ‘nsBind _ (Litv (StrLit s2))’ >>
    ...
    >- ( (* StrLt *)
      gvs[eval_op_SOME] >>
      rename1 ‘nsBind _ (Litv (StrLit s1))’ >>
      rename1 ‘nsLookup _ _ = SOME (Litv (StrLit s2))’ >>
      ntac 6 (qrefine `SUC n` >> simp[cstep_n_def, cstep]) >>
      simp[do_app_def, do_eq_def, lit_same_type_def] >>
      ...
      >- (
        ...
        qexists0 >> simp[step_rel_cases] >> rpt $ goal_assum $ drule_at Any >>
        gvs[env_rel_def, cnenv_rel_def, prim_types_ok_def, bool_type_num_def] >>
        IF_CASES_TAC >> gvs [mlstring_lt_nonrefl]) >>
      ntac 25 (qrefine `SUC n` >> simp[cstep_n_def, cstep, do_app_def]) >>
      qmatch_goalsub_abbrev_tac `cenv' with v := new_env` >>
      `env_ok (cenv' with v := new_env)` by (
        unabbrev_all_tac >> gvs[env_ok_def] >> rpt (irule_at Any EQ_REFL >> simp[])) >>
    ...
    >- ( (* StrGeq *)
      ...
      gvs[env_rel_def, cnenv_rel_def, prim_types_ok_def, Boolv_def] >>
      gvs[mlstring_le_thm,mlstring_ge_thm])
    >- ( (* StrGt *)
      gvs[eval_op_SOME] >>
      rename1 ‘nsBind _ (Litv (StrLit s1))’ >>
      rename1 ‘nsLookup _ _ = SOME (Litv (StrLit s2))’ >>
      ...
      `nsLookup cenv'.c (Short «False») = SOME (0,TypeStamp «False» 0) ∧
       nsLookup cenv'.c (Short «True») = SOME (0,TypeStamp «True» 0)` by
        gvs[env_rel_def, cnenv_rel_def, prim_types_ok_def, bool_type_num_def] >>
      simp[mlstring_gt_le] >> IF_CASES_TAC >> gvs[] >>
      qrefine `SUC n` >> simp[cstep_n_def, cstep] >>
      simp[do_con_check_def, build_conv_def] >>
      qexists0 >> simp[step_rel_cases] >> rpt $ goal_assum $ drule_at Any >>
      gvs[env_rel_def, cnenv_rel_def, prim_types_ok_def, bool_type_num_def]
  ...
  >- ( (* Substring3 - ready to evaluate *)
    ...
    >- (
      ...
      >- (
        qexists0 >> simp[step_rel_cases, SF SFY_ss] >>
        Cases_on ‘s’ >> gvs [implode_def, SF SFY_ss]) >>
      ntac 5 (qrefine `SUC n` >> simp[cstep_n_def, cstep]) >>
      simp[do_app_def, check_type_def, do_arith_def] >>
      ntac 8 (qrefine `SUC n` >> simp[cstep_n_def, cstep]) >>
      simp[do_app_def, do_test_def] >>
      ...
      simp[do_app_def, check_type_def, do_arith_def] >>
      ntac 5 (qrefine `SUC n` >> simp[cstep_n_def, cstep, do_if_def]) >>
      simp[do_app_def, copy_array_def]
      >- (
...
[~Tuple:]
  (LIST_REL (cexp_compile_rel cnenv) ses ces
    ⇒ cexp_compile_rel cnenv (App (Cons «») ses) (Con NONE ces))
...
[~FFI:]
  (cexp_compile_rel cnenv se ce ∧ ch ≠ «»
...
[~TupleCase:]
  (cexp_compile_rel cnenv sce cce ∧ ALL_DISTINCT vs
    ⇒ cexp_compile_rel cnenv (Case sv [«»,vs,sce] NONE)
...
Theorem cexp_var_prefix[simp]:
  cexp_var_prefix cv = var_prefix cv
Proof
  rw[cexp_var_prefix_def, var_prefix_def]
QED

Theorem cexp_pat_row[simp]:
  cexp_pat_row cn vs = pat_row cn vs
Proof
...
Definition cns_ok_def:
  cns_ok ns t ⇔
  cns_arities_ok ns {s | (∃s0. s0 ∈ t ∧ s = s0)}
End
...
Theorem compile_cexp_compile_rel:
  ...
  cns_arities_ok ns { s | ∃s0. s0 ∈ state_cexp$cns_arities e ∧ s = s0 }
...
Proof
  ...
  >- ( (* Letrec *)
    ...
    >- (
      `MAP (λ(a,b,c). var_prefix a) funs =
        MAP var_prefix (MAP FST funs)` by
  ...
  >- ( (* Case *)
    ...
    >- (
      ...
      >- (
        ...
        gvs[EVERY_EL, PULL_EXISTS, MEM_MAP] >>
        last_x_assum drule >> strip_tac >> gvs[] >>
        simp[MEM_EL, PULL_EXISTS] >> goal_assum $ drule_at Any >> simp[] >>
      ...
      >- (
        ...
        PairCases_on `h` >> gvs[] >> Cases_on `h1` >> gvs[]
        >- (Cases_on `t` >> gvs [] >> PairCases_on `h` >> gvs [])
        >- (Cases_on `t` >> gvs [] >> PairCases_on `h'` >> gvs [])
        ) >>
...
Theorem ALOOKUP_cnenv_of_tdefs_NONE:
  ALOOKUP (cnenv_of_tdefs tdefs) k = NONE ⇔
    ¬ MEM k (MAP FST $ FLAT $ MAP SND tdefs)
Proof
...
Theorem ALOOKUP_cnenv_of_tdefs_SOME_lemma[local]:
  ...
  ALOOKUP (FLAT (MAPi (λn (ar,cndefs).
      MAP (λ(cn,ts). (cn, TypeStamp cn (n + m + t_init),
                      LENGTH ts)) cndefs) tdefs)) k =
...
Theorem ALOOKUP_cnenv_of_exns_SOME_lemma[local]:
  ∀exns n cn ts m.
    ...
    ALOOKUP (MAPi (λn (cn,ts).
                     (cn, ExnStamp (n + m + e_init), LENGTH ts)) exns) cn =
