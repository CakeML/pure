...
Theorem barendregt_Apps_EVERY:
  ∀e es.
    barendregt (Apps e es) ⇒
    barendregt e ∧ EVERY barendregt es
Proof
  strip_tac \\ Induct_on `es` using SNOC_INDUCT
  \\ fs [Apps_def,SNOC,Apps_SNOC,EVERY_SNOC,barendregt_alt_def]
QED

Theorem barendregt_Letrec_EVERY:
  ∀e xs.
    barendregt (Letrec xs e) ⇒
      barendregt e ∧
      EVERY (λ(v, e). barendregt e) xs ∧
      DISJOINT (boundvars e) (set (MAP FST xs))
Proof
  strip_tac
  \\ strip_tac
  \\ simp [barendregt_alt_def]
  \\ strip_tac
  \\ fs [DISJOINT_SYM,EVERY_MEM,MEM_MAP,EXISTS_PROD]
  \\ PairCases \\ rw [] \\ res_tac \\ gvs []
QED

Theorem inline_rel_Apps:
  (∀ts us l t1 u1.
    inline_rel l t1 u1 ∧
    LIST_REL (inline_rel l) ts us ⇒
    inline_rel l (Apps t1 ts) (Apps u1 us))
Proof
  Induct
  >- fs [Apps_def]
...
Proof
  ...
  \\ strip_tac \\ gen_tac \\ disch_tac \\ fs []
  \\ gvs [] \\ res_tac \\ fs []
QED
...
Proof
  ...
  \\ fs [lets_for_def]
  \\ irule inline_rel_App
  \\ irule_at Any inline_rel_Lam
  \\ fs [inline_rel_refl]
QED

Theorem lets_for_barendregt:
  ∀vs a.
    barendregt (lets_for c v vs a) ⇒
    barendregt a
Proof
  strip_tac
  \\ Induct_on `vs`
  >- fs [lets_for_def,barendregt_alt_def]
  \\ rw []
  \\ Cases_on `h`
  \\ fs [lets_for_def,barendregt_alt_def]
QED
...
Proof
  ...
  >~ [`Var _ _`] >- (
    gvs [inline_def]
    \\ Cases_on `lookup m v` \\ gvs [inline_rel_refl]
    \\ gvs [inline_rel_refl]
    \\ Cases_on `is_Lam x` \\ gvs [memory_inv_def,inline_rel_refl,exp_of_def]
    \\ Cases_on ‘cl = 0’ \\ gvs []
  )
  >~ [`App _ _ _`] >-
   (pop_assum mp_tac
...
Definition mem_inv_def:
  mem_inv m ns =
    ∀n v. lookup m n = SOME v ⇒ avoid_set_ok ns v
End
...
Definition wf_mem_def:
  wf_mem vars (m:(mlstring, 'a cexp) map) ⇔
    ∀n v. lookup m n = SOME v ⇒
          ∃ce. v = ce ∧ NestedCase_free ce ∧ avoid_set_ok vars ce ∧
  ...
  ∀es es1.
    LIST_REL
      (λce ce'.
         ...
         {a | ∃n x. lookup m n = SOME x ∧ a ∈ freevars (exp_of x)} ∧
         cns_arities ce' ⊆ cns_arities ce ∪
         {a | ∃n x. lookup m n = SOME x ∧ a ∈ cns_arities x}) es es1
    ⇒
    ...
    { a | ∃n x. lookup m n = SOME x ∧ a ∈ cns_arities x }
Proof
  ...
  |> Q.SPEC ‘λm vars cl h ce. ∀ce' vars'.
     ...
     ⇒ block (EVERY NestedCase_free ce' ∧ EVERY cexp_wf ce' ∧
       ...
       LIST_REL (λce ce'.
         ...
         { a | ∃n x. lookup m n = SOME x ∧ a ∈ freevars (exp_of x) } ∧
         cns_arities ce' ⊆ cns_arities ce ∪
         { a | ∃n x. lookup m n = SOME x ∧ a ∈ cns_arities x }) ce ce')’
  |> CONV_RULE (DEPTH_CONV BETA_CONV);
  ...
  >~ [‘Let’] >-
   (gvs [inline_def]
    ...
    \\ ‘wf_mem ns3 (heuristic_insert m h v e1) ∧
        ...
        ∀n y. lookup (heuristic_insert m h v e1) n = SOME y ⇒
              lookup m n = SOME y ∨
  ...
  >~ [‘Letrec’] >-
   (gvs [inline_def]
    ...
    \\ ‘wf_mem ns1 (heuristic_insert_Rec m h vbs) ∧
        ...
        avoid_set_ok ns1 e ∧
        ∀n y. lookup (heuristic_insert_Rec m h vbs) n = SOME y ⇒
              lookup m n = SOME y ∨
              freevars (exp_of y) ⊆
...
Theorem inline_rel_Lams:
  ∀l t u w. inline_rel l t u ⇒ inline_rel l (Lams w t) (Lams w u)
Proof
  Induct_on ‘w’ \\ fs [Lams_def] \\ rw []
  \\ irule inline_rel_Lam
  \\ res_tac \\ fs []
QED

Theorem barendregt_Lams_e:
  ∀vs e. barendregt (Lams vs e) ⇒ barendregt e
Proof
  Induct \\ fs [Lams_def,barendregt_alt_def]
QED
...
Proof
  Cases_on ‘b’ \\ gvs [barendregt_alt_def]
...
Theorem inline_rel_if_lemma:
  inline_rel xs x y ⇒
  inline_rel xs (if b then Seq Fail x else x) (if b then Seq Fail y else y)
Proof
...
Triviality inline_rel_rows_of:
  ∀xs1 ys1.
    ...
    LIST_REL (inline_rel xs) (MAP (SND o SND) xs1) (MAP (SND o SND) ys1) ⇒
    inline_rel xs (rows_of v x xs1) (rows_of v y ys1)
Proof
  Induct \\ Cases_on ‘ys1’ \\ gvs [rows_of_def]
  \\ PairCases_on ‘h’ \\ PairCases \\ gvs [rows_of_def] \\ rw []
  \\ irule inline_rel_Prim \\ gvs [inline_rel_refl]
  \\ irule inline_rel_lets_for \\ fs []
QED
...
Proof
  ...
  >~ [`Var _ _`] >- (
    ...
    >- gvs [inline_rel_refl,exp_of_def]
    \\ gvs [inline_def]
    \\ Cases_on `lookup m v` \\ gvs [inline_rel_refl]
    \\ Cases_on `is_Lam x` \\ gvs [memory_inv_def,inline_rel_refl,exp_of_def]
    \\ Cases_on ‘cl = 0’ \\ gvs []
    ...
    \\ qsuff_tac ‘barendregt (exp_of x) ∧ boundvars (exp_of x) = {} ∧
                  NestedCase_free x ∧
                  letrecs_distinct (exp_of x) ∧ cexp_wf x’
    >- (res_tac \\ fs [pre_def])
    \\ fs []
    \\ Cases_on ‘x’ \\ gvs [cheap_def]
    \\ fs [exp_of_def,is_Lam_def,NULL_EQ,barendregt_alt_def]
  )
  >~ [`App _ _ _`] >- (
    gvs [inline_def]
    \\ rpt (pairarg_tac \\ gvs [])
  ...
  >~ [`Let _ _ _`] >- (
    ...
    >- (
      irule inline_rel_Let
      \\ conj_tac
      >- (
        last_x_assum irule
        \\ fs [cexp_wf_def,pure_expTheory.letrecs_distinct_def]
        \\ fs [DISJOINT_SYM,memory_inv_def,barendregt_alt_def]
      )
      \\ last_x_assum irule
      \\ fs [DISJOINT_SYM]
    ...
    >- (
      ...
      \\ fs [DISJOINT_SYM,memory_inv_def,cexp_wf_def,barendregt_alt_def]
    )
    \\ irule inline_rel_Lam
    \\ last_x_assum irule
    \\ fs [DISJOINT_SYM,barendregt_alt_def]
    \\ imp_res_tac inline_set_of
    \\ imp_res_tac avoid_set_ok_imp_vars_ok
    \\ imp_res_tac SUBSET_TRANS
    \\ gvs []
  ...
  >~ [`Letrec _ _ _`] >- (
    ...
    \\ ‘memory_inv xs m ns2 ∧ avoid_set_ok ns2 e ∧
     ...
     (irule_at Any memory_inv_subset
      ...
      \\ qexists_tac ‘ns’ \\ fs [EVERY_MEM,FORALL_PROD]
      \\ res_tac \\ fs [])
    \\ drule barendregt_Letrec_EVERY \\ strip_tac
  ...
  >~ [`Lam _ _`] >- (
    ...
    \\ rpt (pairarg_tac \\ gvs [])
    \\ gvs [exp_of_def]
    \\ irule_at Any inline_rel_Lams
    \\ last_x_assum irule
    \\ imp_res_tac barendregt_Lams_e
  ...
  >~ [`Prim _ _ _`] >- (
    ...
    \\ rpt (pairarg_tac \\ gvs [])
    \\ gvs [inline_def,exp_of_def]
    \\ irule inline_rel_Prim
    \\ fs [LIST_REL_MAP,o_DEF,FORALL_PROD,LIST_REL_MAP2,PULL_EXISTS]
    \\ last_x_assum irule
    \\ conj_tac >- gvs [barendregt_alt_def,EVERY_MAP]
  ...
  >~ [`Case _ _ _ _ _`] >- (
    ...
    \\ gvs [memory_inv_def,inline_rel_refl,exp_of_def,o_DEF]
    ...
    \\ sg `(∀v1 x1. f = SOME (v1, x1) ⇒ barendregt (exp_of x1))`
    >- (
      rw []
      \\ Cases_on `MEM v (FLAT (MAP (λx. FST (SND x)) bs))`
      >- (
        gvs [barendregt_alt_def]
        \\ qpat_x_assum `barendregt (rows_of _ _ _)` mp_tac
        \\ qid_spec_tac `bs`
        \\ Induct
        >- fs [rows_of_def,IfDisj_def,barendregt_alt_def]
        \\ rw []
        \\ Cases_on `h'`
        \\ Cases_on `r''`
        \\ fs [rows_of_def,IfDisj_def,MAP,barendregt_alt_def]
      )
      \\ gvs [barendregt_alt_def]
      \\ qpat_x_assum `barendregt (rows_of _ _ _)` mp_tac
      \\ qid_spec_tac `bs`
    ...
    \\ sg `EVERY (λe. barendregt (exp_of e)) (MAP (λ(v,vs,e). e) bs)`
    >- (
      rw []
      \\ Cases_on `MEM v (FLAT (MAP (λx. FST (SND x)) bs))`
      >- (
        ...
        \\ Cases_on `h'`
        \\ Cases_on `r''`
        \\ fs [rows_of_def,lets_for_def,IfDisj_def,MAP,MAPi_def,barendregt_alt_def]
    ...
    \\ gvs [if_lemma]
    \\ irule inline_rel_if_lemma
    \\ irule inline_rel_App
    \\ reverse conj_tac
    >-
     (first_x_assum irule \\ fs []
      \\ gvs [cexp_wf_def,pure_expTheory.letrecs_distinct_def,barendregt_alt_def]
      \\ simp [DISJOINT_SYM]
      \\ gvs [SF SFY_ss]
    ...
    >-
     (Cases_on ‘f’ \\ gvs []
      >- (irule inline_rel_Prim \\ fs [])
      \\ PairCases_on ‘x’ \\ gvs []
      \\ rename [‘y ≠ NONE’] \\ Cases_on ‘y’ \\ gvs []
      \\ PairCases_on ‘x’ \\ gvs []
      \\ gvs [IfDisj_def]
      \\ irule inline_rel_Prim \\ fs []
      \\ irule_at Any inline_rel_Prim \\ fs []
      \\ pairarg_tac \\ gvs [inline_rel_refl]
      \\ last_x_assum irule \\ fs []
...
Proof
  rw []
  \\ assume_tac inline_cexp_inline_rel
  \\ gvs []
  \\ last_x_assum irule
  \\ gvs []
  \\ metis_tac []
...
Proof
  ...
  \\ irule exp_eq_trans
  \\ irule_at (Pos last) inline_rel_IMP_exp_eq
