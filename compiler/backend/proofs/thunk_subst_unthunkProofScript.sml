...
Theorem eval_to_mono = eval_to_subst_mono;
...
Proof
      ...
      >- (
        ...
        >- ((* Recclosure-Recclosure *)
          ...
          \\ ‘OPTREL (λ_x _y. is_delay _x ∧ is_delay _y ∧ exp_rel _x _y)
            ...
            by (irule LIST_REL_OPTREL \\ gs []
                ...
                \\ rw [ELIM_UNCURRY])
          \\ gs [OPTREL_def]
          \\ ‘∃y1. _x = Delay y1’ by (Cases_on ‘_x’ \\ gs [])
          \\ gvs [Once exp_rel_cases, dest_anyThunk_def]
          \\ ‘exp_rel (subst_funs f1 y1) (subst_funs g1 x1) ∧
              exp_inv (subst_funs f1 y1) ∧
              exp_ok (subst_funs f1 y1) ∧
              closed (subst_funs f1 y1)’
            by (rw [subst_funs_def, closed_subst]
                >- (
                  irule_at Any exp_rel_subst
                  \\ gvs [LIST_REL_EL_EQN, ELIM_UNCURRY, MAP_MAP_o,
                          combinTheory.o_DEF, EVERY_MAP, EVERY_EL, GSYM FST_THM,
                          SF ETA_ss]
                  \\ irule_at Any LIST_EQ
                ...
                >- (
                  irule_at Any exp_inv_subst
                  \\ gvs [LIST_REL_EL_EQN, ELIM_UNCURRY, MAP_MAP_o,
                          combinTheory.o_DEF, EVERY_MAP, thunk_inv_def,
                          EVERY_EL]
                  \\ dxrule_then strip_assume_tac ALOOKUP_SOME_REVERSE_EL
                  \\ gvs []
                  \\ first_x_assum (drule_then strip_assume_tac) \\ gs [])
                >- (
                  irule_at Any exp_ok_subst
                  \\ gvs [LIST_REL_EL_EQN, ELIM_UNCURRY, MAP_MAP_o,
                          combinTheory.o_DEF, EVERY_MAP, v_ok_def, EVERY_EL]
                  \\ dxrule_then strip_assume_tac ALOOKUP_SOME_REVERSE_EL
                  \\ gvs [] \\ rw []
        ...
        >- (
          ...
          >- (
            ...
            \\ drule_then (qspec_then ‘REVERSE g1’ mp_tac) ALOOKUP_SOME_EL_2
            \\ simp [MAP_REVERSE]
            \\ impl_keep_tac
            >- (
              irule LIST_EQ
            ...
            \\ irule_at Any exp_ok_subst
            \\ irule_at Any LIST_EQ
            \\ gvs [LIST_REL_EL_EQN, ELIM_UNCURRY, MAP_MAP_o, v_ok_def,
                    combinTheory.o_DEF, EVERY_MAP, EVERY_EL, GSYM FST_THM,
                    thunk_inv_def, SF ETA_ss]
            \\ gvs [Once exp_rel_cases, freevars_def]
            \\ first_x_assum (drule_then assume_tac) \\ gs [exp_ok_def]
            \\ first_x_assum (drule_then strip_assume_tac) \\ gvs []
            \\ csimp [EL_MAP, Once thunk_rel_def, EVERY_EL, ELIM_UNCURRY,
                      LIST_REL_EL_EQN]
            \\ rw []
            \\ irule MEM_MAP_f
            \\ rw [EL_MEM])
  ...
  >- ((* Prim *)
    ...
    >- ((* Cons *)
      gvs [exp_inv_def, result_map_def, MEM_MAP, EL_MAP]
      \\ simp [eval_to_def, MAP_MAP_o, combinTheory.o_DEF]
      \\ IF_CASES_TAC \\ gs []
      >- (
        gvs [EVERY_EL, EVERY_MAP, EVERY2_MAP, PULL_EXISTS, exp_ok_def,
             exp_inv_def, eval_to_def, MEM_EL, LIST_REL_EL_EQN, PULL_EXISTS]
      ...
      \\ rw []
      \\ rpt (first_x_assum (drule_all_then strip_assume_tac))
      \\ Cases_on ‘eval_to k (EL n ys)’ \\ gvs [v_ok_def, v_rel_Thunk_def]
      >- (
        irule_at Any thunk_rel_Thunk_Same
        \\ gs [])
      \\ irule_at Any thunk_rel_Thunk_Changed \\ gs []
      \\ first_assum (irule_at Any))
    >- ((* IsEq *)
    ...
    >- ((* AtomOp *)
      ...
      \\ qabbrev_tac ‘f = λj x. case eval_to (j + k - 1) x of
                                ...
                                | _ => INL Type_error’
      \\ simp []
      \\ qsuff_tac ‘∃j. result_map (f j) xs = result_map g ys’
      >- (
        rw []
        \\ qexists_tac ‘j’
        \\ simp [SF ETA_ss]
      ...
      \\ ‘∃j.
            ∀n. n < LENGTH xs ⇒
              ($= +++ (λv w. v_rel v w ∧ v_inv v ∧ v_ok v))
                (eval_to (j + k − 1) (EL n xs)) (eval_to (k − 1) (EL n ys))’
      by (gs [MEM_EL, PULL_EXISTS]
          \\ rpt (pop_assum mp_tac)
          \\ qid_spec_tac ‘ys’
          \\ Induct_on ‘xs’ \\ simp []
          \\ strip_tac
          \\ Cases \\ simp []
          \\ rename1 ‘result_map g (y::ys)’
          \\ rpt strip_tac
          ...
          >- (
            rw []
            \\ ntac 5 (last_x_assum (qspec_then ‘SUC n’ assume_tac)) \\ gs []
            \\ gs [result_map_def, CaseEq "bool", MEM_MAP]
            \\ fs [Once (METIS_PROVE [] “A ⇒ ¬B ⇔ B ⇒ ¬A”)])
          \\ rw []
          \\ ntac 5 (last_x_assum (qspec_then ‘0’ assume_tac)) \\ gs []
          \\ first_x_assum (drule_then (qx_choose_then ‘j1’ assume_tac))
          \\ qexists_tac ‘j1 + j’ \\ gs []
          \\ Cases \\ simp []
          >- (
            drule_then (qspec_then ‘j’ mp_tac) result_rel_mono_left \\ gs []
            \\ impl_tac \\ rw []
            \\ strip_tac \\ gs []
          ...
          \\ strip_tac
          \\ first_x_assum (drule_then assume_tac)
          \\ drule_then (qspec_then ‘j1’ mp_tac) result_rel_mono_left \\ gs []
          \\ impl_tac \\ rw []
          \\ strip_tac \\ gs []
      ...
      \\ fs [Once (METIS_PROVE [] “A ⇒ ¬B ⇔ B ⇒ ¬A”)]
      \\ gvs [EL_MAP] \\ rw []
      \\ ntac 5 (first_x_assum (drule_all_then assume_tac))
      \\ unabbrev_all_tac \\ gs []
      \\ rpt CASE_TAC \\ gs []))
QED
