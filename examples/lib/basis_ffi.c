...
void ffistdout (unsigned char *c, long clen, unsigned char *a, long alen) {
    for (int i=0;i<clen;i++) {
        putc(c[i],stdout);
    }
    a[0] = 0;
...
void fficline_arg (unsigned char *c, long clen, unsigned char *a, long alen) {
    int index = clen;
    if (index < argc) {
        write_ret_string(a,alen,argv[index]);
    } else {
        a[0] = 0;
        a[1] = 0;
    }
