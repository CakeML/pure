-- nofib: spectral/sorting
-- PureLang port adapted from PureCake examples/quicksort.hs. Corresponds to the
-- nofib `sorting` benchmark (spectral subset; orig. Will Partain). Sorts the
-- descending list [n..0] with a functional quicksort and, in place, with an
-- imperative array quicksort, checking both outputs are sorted. The single
-- command-line argument is the list/array size n.
