...
Theorem pure_vwalk_ind:
  ∀s. pure_wfs s ⇒
    ∀P. (∀v. (∀w u. FLOOKUP s v = SOME (CVarCons u []) ⇒ P u) ⇒ P v) ⇒ ∀v. P v
Proof
  ...
  ∀s. pure_wfs s ⇒
    ∀v.
      pure_vwalk s v =
        case FLOOKUP s v of
        | NONE => CVarCons v []
        | SOME (CVarCons w []) => pure_vwalk s w
        | SOME t => t
Proof
  ...
  Cases_on `x` using itype_SNOC_CASES >>
  gvs[encode_itype_alt,decode_encode,decode_utype_acc_def] >>
  simp[list_case_eq] >>
  metis_tac[list_nchotomy,APPEND_eq_NIL]
QED
...
Theorem pure_walk:
  ∀s t. pure_walk s t =
    case t of
    | CVarCons v [] => pure_vwalk s v
    | t => t
Proof
  rw[pure_walk_def, pure_vwalk_def, walk_def] >>
  Cases_on `t` using itype_SNOC_CASES >>
  simp[encode_itype_alt] >>
  gvs[decode_utype_def,decode_utype_acc_def,decode_encode] >>
  simp[list_case_eq] >>
  metis_tac[list_nchotomy,APPEND_eq_NIL]
QED
...
Triviality SNOC_CONS:
  ∀t ts. ?h l. SNOC t ts = CONS h l
Proof
...
Proof
  ...
  Cases_on `x` using itype_SNOC_CASES >>
  gvs[encode_itype_alt] >>
  qspecl_then [`t`,`ts`] strip_assume_tac SNOC_CONS >>
  simp[] >>
  ...
  ∀s. pure_wfs s ⇒
    ∀t v.
      pure_oc s t v =
        ...
        | CVarCons u [] => v = u
        | CVarCons u its => (pure_oc s (CVarCons u []) v) ∨
            EXISTS (λit. pure_oc s it v) its
        | TypeCons n its => EXISTS (λit. pure_oc s it v) its
        | DBVarCons n its => EXISTS (λit. pure_oc s it v) its
        | _ => F
Proof
  rw[pure_oc_def] >>
  ...
  Cases_on `t` using itype_SNOC_CASES >>
  simp[encode_itype_alt, decode_utype_def,
    decode_utype_acc_def, decode_encode,
    encode_vwalk, pure_oc_lemma] >>
  Cases_on `pure_vwalk s n` using itype_SNOC_CASES >>
  rw[encode_itype_alt, pure_oc_lemma, EXISTS_SNOC, DISJ_COMM,
    encode_vwalk] >>
  Cases_on `ts` >>
  simp[EXISTS_SNOC] >>
  metis_tac[]
  ...
  ∀t1 t2 s.
    ...
    pure_unify s t1 t2 =
      ...
      | (DBVarCons db1 ts1, DBVarCons db2 ts2) =>
          if db1 = db2 then pure_unifyl s ts1 ts2 else NONE
