...
Theorem boundvars_freshen:
  ∀avoid x body.
    freevars body ⊆ set avoid ∧
    MEM x (boundvars (freshen avoid body))
    ⇒ ¬MEM x avoid
Proof
  rw[] >>
  drule freshen_safe_renaming >> gvs[] >>
  gvs[EXTENSION, DISJOINT_DEF] >>
  metis_tac[]
QED
...
Theorem subst_single_distrib:
  ∀ body f v arg.
    (∀n v. FLOOKUP f n = SOME v ⇒ closed v) ∧
    DISJOINT (freevars arg) (boundvars body)
  ⇒ subst f (subst v arg body) = subst v (subst f arg) (subst (f \\ v) body)
Proof
